...
public final class QueryUtils {
    ...
    /**
     ** Return a list of {@link items} objects that has been built up from

     parsing a JSON response
     */
    public static ArrayList extractEarthquakes() {
        ...
        try {
            ...
            for (int i = 0; i<earthquakeArray.length(); i++)
            {
                ...
                JSONObject properities = feature.getJSONObject("properties");

                Double magnitude = properities.getDouble("mag");

                String place = properities.getString("place");

                long time = properities.getLong("time");

                String url = properities.getString("url");


                earthquakes.add(new EarthquakeList(magnitude,place,time, url));
            }
